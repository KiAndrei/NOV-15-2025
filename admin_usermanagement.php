...
<head>
    ...
    <style>
        ...
        .alert-modal-details {
            ...
            border: 1px solid rgba(124, 15, 47, 0.12);
            border-radius: 12px;
            padding: 14px 16px;
            margin-bottom: 16px;
        }
        ...
        .alert-modal-footer.two-buttons {
            display: flex;
            justify-content: center;
            align-items: center;
            gap: 12px;
        ...
        .alert-modal-btn:disabled::before {
            display: none;
        }
...
<body>
    ...
    <div class="main-content">
        ...
        <div style="background: linear-gradient(135deg, #fff 0%, #f8fafc 100%); border-radius: 16px; box-shadow: 0 4px 16px rgba(0,0,0,0.05); margin-bottom: 24px; padding: 24px; border: 1px solid rgba(255,255,255,0.8);">
            <div class="search-filter-container" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
                ...
                <tbody>
                    ...
                    <?php else: foreach ($attorneys as $user): ?>
                        <tr data-id="<?= $user['id'] ?>">
                            ...
                            <td>
                                <button type="button" onclick="confirmDeleteUser(<?= $user['id'] ?>, '<?= htmlspecialchars($user['name']) ?>', 'Attorney')" class="action-btn btn-delete">
    ...
    <script>
        ...
        function closeDeleteUserModal(modalId = 'deleteConfirmModal') {
        ...
        function closeAllDeleteModals() {
            ['deleteWarningModal', 'deleteFinalModal', 'deleteConfirmModal'].forEach(id => closeDeleteUserModal(id));
        }
        
        function submitDeleteUser(userId, triggerBtn) {
            if (triggerBtn) {
        ...
        function openDeleteWarningModal(userId, userName, userType) {
            closeDeleteUserModal('deleteWarningModal');
            
            const modal = document.createElement('div');
            modal.className = 'alert-modal';
            modal.id = 'deleteWarningModal';
            modal.innerHTML = `
                <div class="alert-modal-content warning">
                    <div class="alert-modal-header">
                        <div class="alert-modal-icon warning">
                            <i class="fas fa-exclamation-circle"></i>
                        </div>
                        <h3 class="alert-modal-title">Delete User Account</h3>
                        <p class="alert-modal-message">
                            You are about to delete <strong>${escapeHtml(userName)}</strong> (${escapeHtml(userType === 'Admin_attorney' ? 'Attorney' : userType)}). This will permanently remove their access and related records.
                        </p>
                    </div>
                    <div class="alert-modal-body">
                        <p style="margin:0; color:#4b5563; line-height:1.5;">
                            This action is irreversible. Please confirm you want to proceed to the next confirmation step.
                        </p>
                    </div>
                    <div class="alert-modal-footer two-buttons">
                        <button type="button" class="alert-modal-btn cancel" id="deleteWarnCancel">
                            <i class="fas fa-times"></i> Cancel
                        </button>
                        <button type="button" class="alert-modal-btn danger" id="deleteWarnContinue">
            ...
            const cancelBtn = modal.querySelector('#deleteWarnCancel');
            const continueBtn = modal.querySelector('#deleteWarnContinue');
            cancelBtn.addEventListener('click', () => closeDeleteUserModal('deleteWarningModal'));
            continueBtn.addEventListener('click', () => {
                closeDeleteUserModal('deleteWarningModal');
                openDeleteFinalModal(userId, userName, userType);
            });
        ...
        function openDeleteFinalModal(userId, userName, userType) {
            closeDeleteUserModal('deleteFinalModal');
            
            const modal = document.createElement('div');
            modal.className = 'alert-modal';
            modal.id = 'deleteFinalModal';
            modal.innerHTML = `
                <div class="alert-modal-content warning">
                    <div class="alert-modal-header">
                        <div class="alert-modal-icon warning">
                        ...
                        <h3 class="alert-modal-title">Final Confirmation</h3>
                        <p class="alert-modal-message">
                            This is your final confirmation before deleting <strong>${escapeHtml(userName)}</strong> (${escapeHtml(userType === 'Admin_attorney' ? 'Attorney' : userType)}).
                        </p>
                    </div>
                    <div class="alert-modal-body">
                        <ul style="margin:0; padding-left:18px; color:#4b5563;">
                            <li>All associated records will be permanently removed.</li>
                            <li>The user will lose access immediately.</li>
                            <li>This operation cannot be reversed.</li>
                        </ul>
                    </div>
                    <div class="alert-modal-footer two-buttons">
                        <button type="button" class="alert-modal-btn cancel" id="deleteFinalCancel">
                            <i class="fas fa-times"></i> Cancel
                        </button>
                        <button type="button" class="alert-modal-btn danger" id="deleteFinalContinue">
                            <i class="fas fa-shield-alt"></i> Proceed
                        </button>
                    </div>
                </div>
            `;
            ...
            const cancelBtn = modal.querySelector('#deleteFinalCancel');
            const continueBtn = modal.querySelector('#deleteFinalContinue');
            cancelBtn.addEventListener('click', () => closeDeleteUserModal('deleteFinalModal'));
            continueBtn.addEventListener('click', () => {
                closeDeleteUserModal('deleteFinalModal');
                openDeleteConfirmModal(userId, userName, userType);
            });
        ...
        function openDeleteConfirmModal(userId, userName, userType) {
            const safeName = escapeHtml(userName);
            const roleLabel = userType === 'Admin_attorney' ? 'Attorney' : userType;
            const safeType = escapeHtml(roleLabel);
            const safeTypeLower = escapeHtml((roleLabel || '').toLowerCase());
            
            closeDeleteUserModal('deleteConfirmModal');
            ...
            modal.innerHTML = `
                <div class="alert-modal-content warning">
                    <div class="alert-modal-header">
                        ...
                        <h3 class="alert-modal-title">Confirm User Deletion</h3>
                        <p class="alert-modal-message">
                            Deleting this ${safeTypeLower} account will permanently remove access and associated records. This action cannot be undone.
                        </p>
                    </div>
                    <div class="alert-modal-body">
                        ...
                        <label class="alert-modal-label" for="deleteConfirmInput">Type <strong style="color:#b91c1c;">DELETE</strong> to continue:</label>
                        <input type="text" id="deleteConfirmInput" class="alert-modal-input" placeholder="DELETE" autocomplete="off" />
                        <p class="alert-modal-hint">This action is permanent and cannot be undone.</p>
                    </div>
                    <div class="alert-modal-footer two-buttons">
                        <button type="button" class="alert-modal-btn cancel" id="deleteCancelBtn">
                            <i class="fas fa-times"></i> Keep User
                        </button>
                        <button type="button" class="alert-modal-btn danger" id="deleteConfirmBtn" disabled>
            ...
            input.addEventListener('keydown', (event) => {
                if (event.key === 'Enter' && !confirmBtn.disabled) {
                    submitDeleteUser(userId, confirmBtn);
                }
            });
            
            confirmBtn.addEventListener('click', () => submitDeleteUser(userId, confirmBtn));
        ...
        function confirmDeleteUser(userId, userName, userType) {
            ...
            openDeleteWarningModal(userId, userName, userType);
        }
</script>
