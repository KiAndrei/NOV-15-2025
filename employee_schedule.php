...
if (isset($_POST['action']) && $_POST['action'] === 'add_event') {
    ...
    if ($stmt->affected_rows > 0) {
        ...
        if ($conn->query("SHOW TABLES LIKE 'notifications'")->num_rows) {
            ...
            $employee_result = $stmt_employee->get_result()->fetch_assoc();
            $employee_name = $employee_result ? $employee_result['name'] : 'Employee';
...
<body>
        ...
        <ul class="sidebar-menu">
            ...
            <li class="has-submenu">
                ...
                <ul class="submenu">
                    ...
                    <li><a href="employee_send_files.php"><i class="fas fa-paper-plane"></i><span>Sent Files</span></a></li>
                </ul>
            </li>
    ...
    <style>
        ...
        .sidebar {
            ...
            color: #ffffff;
            box-shadow: 4px 0 15px rgba(93, 14, 38, 0.3);
            padding: 20px 0;
        }
        ...
        .hamburger-menu {
            ...
            padding: 6px;
            cursor: pointer;
            box-shadow: 0 6px 18px rgba(93, 14, 38, 0.28);
        ...
        .hamburger-menu:hover {
            ...
            box-shadow: 0 10px 24px rgba(93, 14, 38, 0.35);
        }
        ...
        .action-buttons .btn,
        .action-buttons .view-options .btn {
            padding: 6px 12px;
            font-size: 11px;
            border-radius: 5px;
        }
        ...
        .action-buttons .view-options .btn i {
            font-size: 0.85rem;
        }
        ...
        function initializeCalendarFunctions(calendar) {
            ...
            if (saveWalkinBtn) {
                ...
                .then(result => {
                    ...
                    // Clean result from PHP warnings/notices (strip HTML tags and warnings)
                    const cleanResult = result.replace(/<br\s*\/?>/gi, '').replace(/<b>Warning<\/b>:[^<]*/gi, '').replace(/<b>Notice<\/b>:[^<]*/gi, '').replace(/<[^>]*>/g, '').trim();
                    console.log('Walk-in cleaned response:', cleanResult); // Debug log
                    
                    // Check if response contains 'success' (even if there are warnings)
                    if (cleanResult === 'success' || cleanResult.includes('success')) {
                        ...
                        // Handle specific error messages
                        if (cleanResult.startsWith('error:') || result.includes('error:')) {
                            const errorMatch = cleanResult.match(/error:\s*(.+)/i) || result.match(/error:\s*(.+)/i);
                            const errorMsg = errorMatch ? errorMatch[1].trim() : 'Unknown error occurred';
                            showNotification('❌ ' + errorMsg, 'error');
                        } else {
