...
if (isset($_POST['action']) && $_POST['action'] === 'edit_event') {
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $stmt_info = $conn->prepare("SELECT cs.client_id, cs.walkin_client_name, cs.attorney_id, uf.user_type AS attorney_user_type FROM case_schedules cs LEFT JOIN user_form uf ON cs.attorney_id = uf.id WHERE cs.id = ?");
        ...
        $attorney_user_type = strtolower($schedule_info['attorney_user_type'] ?? '');
        if ($schedule_info['attorney_id'] && $attorney_user_type === 'attorney' && $conn->query("SHOW TABLES LIKE 'notifications'")->num_rows) {
...
if (isset($_POST['action']) && $_POST['action'] === 'add_event') {
    ...
    if ($stmt->affected_rows > 0) {
        ...
        // Get attorney info for notifications/audit
        $attorney_name = '';
        $attorney_user_type = '';
        $stmt_attorney_info = $conn->prepare("SELECT name, user_type FROM user_form WHERE id = ?");
        $stmt_attorney_info->bind_param('i', $selected_user_id);
        $stmt_attorney_info->execute();
        if ($row = $stmt_attorney_info->get_result()->fetch_assoc()) {
            $attorney_name = $row['name'] ?? '';
            $attorney_user_type = strtolower($row['user_type'] ?? '');
        }
        
        // Notify attorney about the new schedule (only when target is an attorney account)
        if ($attorney_user_type === 'attorney' && $conn->query("SHOW TABLES LIKE 'notifications'")->num_rows) {
        ...
        try {
            // Ensure attorney name is available for logging
            $attorney_name_for_log = $attorney_name;
            if ($attorney_name_for_log === '') {
                ...
                if ($attorney_row && !empty($attorney_row['name'])) { $attorney_name_for_log = $attorney_row['name']; }
            }
            ...
            $details = "Created by: $admin_name; Attorney: " . ($attorney_name_for_log !== '' ? $attorney_name_for_log : (string)$selected_user_id) . "; Client: " . ($client_name !== '' ? $client_name : ($client_id ? (string)$client_id : 'N/A')) . "; Type: $type; Date: $date; Time: $start_time-$end_time; Location: $location";
...
<head>
    ...
    <style>
        ...
        .sidebar-header, body .sidebar-header {
        ...
        .sidebar-header img, body .sidebar-header img {
            margin-left: auto !important;
            margin-right: auto !important;
        }
        .sidebar-header h2, body .sidebar-header h2 {
            width: 100%;
        }
        .sidebar-overlay {
            position: fixed;
    ...
    <script>
        ...
        (function(){
            ...
            function setupGridPagination(grid){
                ...
                // Check if this is the completed grid or upcoming events grid
                const isCompletedGrid = grid.classList.contains('completed-events-grid');
                
                // Get all cards, but filter out completed ones for upcoming events grids
                let allCards = Array.from(grid.querySelectorAll('.event-card'));
                
                // For upcoming events grids, filter out completed schedules
                if (!isCompletedGrid) {
                    allCards = allCards.filter(card => {
                        // Check if card is in completed section
                        if (card.closest('.completed-events-grid')) {
                            return false;
                        }
                        // Check if card has completed class
                        if (card.classList.contains('completed-schedule')) {
                            return false;
                        }
                        // Check if status select value is "Completed"
                        const statusSelect = card.querySelector('.status-select');
                        if (statusSelect && statusSelect.value.toLowerCase() === 'completed') {
                            return false;
                        }
                ...
                // Use a variable that can be re-evaluated
                let cards = allCards;
                
                // If no cards after filtering, check if pagination already exists and remove it
                if (cards.length === 0) {
                    const existingPager = grid.parentElement.querySelector('.pagination-bar');
                    if (existingPager) {
                ...
                function render(){
                    // Re-filter cards on each render to get current state
                    if (!isCompletedGrid) {
                        let currentCards = Array.from(grid.querySelectorAll('.event-card'));
                        cards = currentCards.filter(card => {
                            // Exclude if in completed section
                            if (card.closest('.completed-events-grid')) {
                                return false;
                            }
                            // Exclude if has completed class
                            if (card.classList.contains('completed-schedule')) {
                                return false;
                            }
                            // Exclude if status is completed
                            const statusSelect = card.querySelector('.status-select');
                            if (statusSelect && statusSelect.value.toLowerCase() === 'completed') {
                                return false;
                            }
                        ...
                        // For completed grid, get all cards (but still check they're actually completed)
                        cards = Array.from(grid.querySelectorAll('.event-card')).filter(card => {
                            return card.classList.contains('completed-schedule') || 
                                   (card.querySelector('.status-select') && 
                                    card.querySelector('.status-select').value.toLowerCase() === 'completed');
                        });
                    }
                    
                    // If no cards left, hide pagination
                    if (cards.length === 0) {
                ...
                next.onclick = ()=>{ if(currentPage < Math.ceil(cards.length / perPage)) { currentPage++; render(); } };
            ...
            // Expose setupGridPagination globally for refresh calls
            window.setupGridPagination = setupGridPagination;
        })();
    </script>
...
<style>
    ...
    <style>
        ...
        @media (max-width: 900px) {
            ...
            .calendar-container #calendar { min-width: 720px; }
        }
    ...
    <script>
        ...
        function moveCardToCompletedSection(eventCard) {
            ...
            if (completedGrid) {
                // Find all upcoming events grids to refresh their pagination
                const upcomingGrids = document.querySelectorAll('.events-grid:not(.completed-events-grid)');
                ...
                // Refresh pagination for all grids after card is moved
                setTimeout(() => {
                    // Remove all existing pagination bars
                    document.querySelectorAll('.pagination-bar').forEach(pager => pager.remove());
                    
                    // Re-setup pagination for all upcoming events grids (will automatically filter out completed)
                    document.querySelectorAll('.events-grid:not(.completed-events-grid)').forEach(grid => {
                        if (window.setupGridPagination) {
                            window.setupGridPagination(grid);
                        }
                    });
                    
                    // Re-setup pagination for completed grid
                    if (window.setupGridPagination) {
                        window.setupGridPagination(completedGrid);
                    }
                    
                    console.log('✅ Refreshed all paginations after moving card');
                }, 300);
            } else {
